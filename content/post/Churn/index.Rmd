...
* Vemos que la variable International_calls tiene 76 valores perdidos.  

* La variable International_charges tiene 66 valores perdidos.

* 12 de estos valores perdidos se encuentran en el mismo usuario.
...
* Se observa la distribución de la data:
  ...
  + International calls presenta una distribución con asimetría positiva. Se procede a usar un algoritmo de k-nearest neighbors.

  + Por el contrario, International charges tiende a converger a una distribución normal, por ende, se opta por imputar los valores mediante la estimación de una regresión lineal de esta variable en función de International calls.  
...
* La media de los datos originales de las International_calls era de 4.479, en el caso de los datos imputados resulta ser de 4.485. 

* Respecto a International_charges, la media de los datos originales era de 2.765, mientras que la imputada es de 2.769. En ese sentido, se resalta que los datos imputados son cercanos a los originales.
...
* Se normalizan las variables con el objetivo de tenerlas en la misma escala.

* Para la modelización de los datos se estima un modelo ensamblador ya que presenta la ventaja de   mejorar la precisión, estabilidad del modelo y evitar overfitting. Se utiliza la técnica de bagging para disminuir la varianza de las predicciones. Esta se basa en la combinación de las predicciones de múltiples modelos "débiles" para formar un modelo robusto. Cada uno de los modelos se elabora en base a diferentes remuestreos con reemplazo de la población. Para tales fines se modeliza con el random forest, en el cual, se crean múltiples árboles de decisión para formar todo un bosque de árboles. En este se seleccionan de entre todos los predictores una muestra aleatoria de "m" predictores como candidatos para cada modelo. En ese tenor, es una combinación de remuestreo de observaciones y de predictores.

* La métrica a maximizar es el AUC y se procede a realizar el tuning de los hiperparámetros. 
...
Para información sobre los códigos utilizados, acceder a [Github](https://github.com/Eduardovasquezn/Modelo-Churn-Telecomunicaciones).
